...
Safety scans read a persisted SQLite change counter instead of rereading and hashing the whole library on the server thread. Transactional triggers advance it for media or collection inserts, deletes, and actual content changes (including availability, technical properties, ratings and overrides); identical writes and bookkeeping timestamps do not. The diagnostic phase is `change-check`. Trigger changes roll back with the original transaction and cover writes from other database connections. The first scan and changed scans refresh playback caches and automatic lineups. Unchanged scans retain their prepared caches and workers. A failed completion listener keeps the previous change token so the next scan retries. Offline roots still invalidate schedules immediately, and metadata maintenance remains enabled.
...
Local verification (2026-09-25): on a private copy containing 24,642 media rows, 1,000 revision reads averaged 0.005 ms per read. Re-upserting every media row in 250-row batches preserved the token (639 ms total, 9.2 ms slowest batch). The previous local full-catalog fingerprint took about 148 ms. These are local measurements, not Unraid latency guarantees; compare live incidents after deployment. Regression tests cover no-op writes, collection reconciliation, availability, removal, collection overrides, transaction rollback, reopen persistence, and external-connection writes.
