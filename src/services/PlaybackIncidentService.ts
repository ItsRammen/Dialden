...
export class PlaybackIncidentService {
  ...
  constructor(private readonly path: string, private readonly context?: (channelId: string, sessionId?: string) => Record<string, string | number | null> | Promise<Record<string, string | number | null>>) {
  ...
  async record(clientId: string, input: unknown): Promise<string[]> {
    ...
    for (const raw of input.slice(0, 2)) {
      ...
      if (typeof row.channelId === 'string' && this.context) Object.assign(row, await this.context(row.channelId, typeof row.sessionId === 'string' ? row.sessionId : undefined))
      batch.push(row)
    }
